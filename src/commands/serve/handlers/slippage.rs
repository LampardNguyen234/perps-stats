...
use crate::commands::serve::{
    ...
    models::responses::{DataResponse, PaginatedResponse, PaginationMeta},
    state::AppState,
};
...
impl SlippageQuery {
    ...
    pub fn validate(&self) -> Result<(), AppError> {
        ...
        if symbol.trim().is_empty() {
            return Err(AppError::bad_request(
                "Symbol parameter is required and cannot be empty".to_string(),
            ));
        }
        ...
        if !symbol.chars().all(|c| c.is_alphanumeric()) || symbol.len() > 20 {
            return Err(AppError::bad_request(
                "Symbol must be alphanumeric and max 20 characters".to_string(),
            ));
        }
        ...
        if let Some(exchange) = &self.exchange {
            if exchange.trim().is_empty() {
                return Err(AppError::bad_request(
                    "Exchange parameter cannot be empty".to_string(),
                ));
            }
            if !exchange.chars().all(|c| c.is_alphanumeric() || c == '_') || exchange.len() > 50 {
                return Err(AppError::bad_request(
                    "Exchange must be alphanumeric (with underscores) and max 50 characters"
                        .to_string(),
                ));
            }
        }
        ...
        let amount = match &self.amount {
            ...
            None => {
                return Err(AppError::bad_request(
                    "Amount parameter is required".to_string(),
                ))
            }
        };
        ...
        if amount.trim().is_empty() {
            return Err(AppError::bad_request(
                "Amount parameter is required and cannot be empty".to_string(),
            ));
        }
...
#[allow(dead_code)]
fn filter_slippages(
    slippages: Vec<Slippage>,
    amount_filter: Option<Decimal>,
...
fn filter_slippages_with_exchange(
    slippages: Vec<SlippageWithExchange>,
    amount_filter: Decimal,
) -> Vec<SlippageWithExchange> {
    slippages
        .into_iter()
        .filter(|slippage_with_exchange| {
...
pub async fn get_slippage_history(
    ...
    let (total_count, slippages): (i64, Vec<SlippageWithExchange>) = match &query.exchange {
        ...
        Some(exchange) => {
            ...
            let slippage_with_exchange: Vec<SlippageWithExchange> = slippage_data
                .into_iter()
                .map(|s| SlippageWithExchange {
                    exchange: exchange.clone(),
                    slippage: s,
            ...
            // Apply filters
            let filtered_data =
                filter_slippages_with_exchange(slippage_with_exchange, amount_filter);
        ...
        None => {
            ...
            let symbol = query.symbol();
            let mut data_sql = sqlx::query(&data_query).bind(symbol).bind(start).bind(end);
